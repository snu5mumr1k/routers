...
class TestCase(FlaskTestCase):
    def test_location_create(self):
        ...
        db.session.commit()
        self.assertTrue(
                len(Location.query.filter(Location.address == address).all())
            )
        self.assertTrue(
                len(Location.query.filter(Location.id == location.id).all())
            )
        self.assertEqual(
    ...
    def test_location_update(self):
        ...
        self.assertTrue(
                len(Location.query.filter(Location.id == location_id).all())
            )
        address += ", Red Square"
    ...
    def test_location_routers(self):
        self.assertTrue(len(ROUTER_MODELS))
        routers_cnt = 15
        ...
        self.assertTrue(start_location)
        self.assertTrue(cur_location)
        self.assertEqual(len(routers), routers_cnt)
        self.assertEqual(len(start_location.routers), routers_cnt)
        ...
        self.assertEqual(
                len(start_location.routers),
                routers_cnt - routers_cnt // 2
            )
        self.assertEqual(
                len(cur_location.routers),
                routers_cnt // 2
            )
